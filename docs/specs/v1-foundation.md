...
- Local live storage defaults to `LodgeOffice.db` in the project folder (legacy `Treasurer.db` if still present), or can be overridden with `TREASURER_DATABASE` / `LODGE_OFFICE_DATABASE` in `config.local`
